document.addEventListener("DOMContentLoaded", () => {
  ...
  // Initialize attacker positions (multiple attackers around the edges)
  for (let i = 0; i < 8; i++) {
  ...
  function drawAnimation() {
    ...
    // Draw server label
    // animCtx.fillStyle = "#00ffff"
    // animCtx.fillText("SERVER", w / 2, h / 2 + 50)
  ...
  function step() {
    ...
    const regions = [0, 0, 0, 0, 0]
    const dist = [0.3, 0.25, 0.2, 0.15, 0.1]
    for (let i = 0; i < 5; i++) {
      regions[i] = dist[i] * totalBots * (0.9 + Math.random() * 0.2)
    }

    const incoming = regions.reduce((s, r) => s + r * per * (0.8 + Math.random() * 0.4), 0)
    ...
    regions.forEach((r, i) => {
      ...
      el.style.top = 20 + i * 8 + "%"
      el.style.width = Math.min(40, 8 + r / 1000) + "px"
      el.style.height = el.style.width
      el.style.borderRadius = "50%"
      el.style.background = "radial-gradient(circle at 30% 30%, #00ffff, #0088aa)"
      el.style.boxShadow = "0 0 20px rgba(0, 255, 255, 0.6)"
      mapEl.appendChild(el)
    })

    totalEl.textContent = fmt(Math.round(allowed)) + " req/s"
